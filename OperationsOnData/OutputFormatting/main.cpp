...
//After std::flush, we're sure that at this line, the message has been sent to the stream.
//This may be important in some applications.

std::cout << "-------------------------------" << std::endl;
std::cout << std::endl;

//std setw(); Adjusts the field width for the item to br printed.
...
std::cout << "Formatted table : " << std::endl; //Data is right justified by default
...
//std::boolalpha noboolalpha : control bool output format : 1/0 or true/false
bool condition {true};
...
//Showpos : shows the '+' sign for positive numbers. noshowpos reverts this back to default
int pos_num {34};
int neg_num {-45};
...
//Different number systems/bases : std::dec, std::oct, std::hex
//Only has effect on integral types. No effect on floating types as shown in the double below.
int pos_int {717171};
int neg_int {-47347};
...
std::cout.unsetf (std::ios::scientific | std::ios::fixed); //Hack, no official way to reset
std::cout << "a : " << a << std::endl; 
std::cout << "b : " << b << std::endl; 
std::cout << "c : " << c << std::endl; 
...
//setprecision() : the number of digits printed out for a floating type. Default is 6.
//If the precision is bigger than supported by the type, you'll just print garbage.
//Double ~ 15 so after 15 numbers, the rest is garbage if the precision is set higher than 15
...
std::cout << "a (reset to default precision(6)) : " << a << std::endl;

std::cout << "-------------------------------" << std::endl;
std::cout << std::endl;

//Showpoint : Forces a decimal point and trailing zeros where necessary. default is noshowpoint
double d {34.1};
double e {101.99};
double f {12.0};
int g {45};
